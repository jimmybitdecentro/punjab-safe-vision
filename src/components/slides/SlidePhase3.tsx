...
export default function SlidePhase3() {
  return (
    <SlideLayout dark className="slide-gradient-crimson relative overflow-hidden">
      <img src={heroHimachalMap} alt="" className="absolute inset-0 w-full h-full object-cover opacity-15" />
      ...
      <div className="space-y-10 relative z-10">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {[
            { icon: Globe, title: "Full Himachal Coverage", desc: "Every district, every zone — complete state-wide surveillance and intelligence network" },
            ...
            { icon: Monitor, title: "Central Command Center", desc: "CM-level visibility with live Himachal map, district performance tracking in real-time" },
          ].map((f, i) => (
        ...
        <div className="rounded-xl overflow-hidden border border-primary-foreground/10">
          <div className="relative h-32">
            <img src={heroHimachalMap} alt="Himachal network map" className="w-full h-full object-cover opacity-40" loading="lazy" width={1920} height={1080} />
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
            <p className="absolute bottom-3 left-4 text-xs uppercase tracking-widest opacity-60">Himachal Command Center — Live View</p>
          </div>
        ...
        <p className="text-center text-lg font-medium opacity-70 italic">
          "Transform Himachal into a data-driven, drug-free state"
        </p>
      </div>
    </SlideLayout>
  );
